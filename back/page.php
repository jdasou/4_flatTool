...
if (isset($_SESSION["id_compte"])) {
        ...
        switch ($_GET["cas"]) {
            case "inserer_page":
                if (empty($_POST["titre_page"])) {
                    ...
                    $requete =
                        "INSERT INTO pages SET titre_page='" .
    ...
    while ($ligne = mysqli_fetch_object($resultat)) {
        ...
        $content .= "<summary>";
        $content .=
            "<div>" . $ligne->id_page . " - " . $ligne->titre_page . "</div>";
        if ($ligne->visible == 1) {
            $content .=
                "<div class=\"actions\"><a href=\"back.php?action=page&cas=changer_etat&etat=0&id_page=" .
                $ligne->id_page .
                "\"><span class=\"dashicons dashicons-visibility txt-3\"></span></a>";
        } else {
            $content .=
                "<div class=\"actions\"><a href=\"back.php?action=page&cas=changer_etat&etat=1&id_page=" .
                $ligne->id_page .
                "\"><span class=\"dashicons dashicons-hidden txt-3\"></span></a>";
        }
        $content .=
            "<a class='p-1  ' href=\"back.php?action=page&cas=recharger_page&id_page=" .
            $ligne->id_page .
            "#form_back\"><span class=\"dashicons dashicons-admin-customizer txt-3\"></span></a>";
        $content .=
            "<a href=\"back.php?action=page&cas=avertir_page&id_page=" .
            $ligne->id_page .
            "\"><span class=\"dashicons dashicons-no txt-3\"></span></a></div>";
        $content .= "</summary>";

        $content .=
            "<div class=\"all\">Créée le : " .
            $ligne->date_page .
            "<br><br>" .
            $ligne->contenu_page .
            "</div>";
