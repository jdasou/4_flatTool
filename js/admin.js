...
$(document).ready(function () {
  /**
   * Insertion des nombre slider
   */
  window.transferedText = false;
  // let slider_input = $('#pages_slide');
  let slider_field = document.getElementById("pages_slide");
  if (slider_field) {
    slider_field.addEventListener("dragover", function (e) {
      e.preventDefault();
    });
    slider_field.addEventListener("drop", function (e) {
      if (window.transferedText) {
        if (this.value != "") {
          this.value += "-" + window.transferedText;
        } else {
          this.value = window.transferedText;
        }
        window.transferedText = false;
  ...
  let iframe = document.querySelector('[name="aide"]');
  iframe.addEventListener("load", function () {
    let helpDocument = iframe.contentDocument || iframe.contentWindow.document;
    let slider_number = helpDocument.querySelectorAll("h3[draggable]");
    if (slider_number) {
      for (let number of slider_number) {
        number.addEventListener("drag", function (e) {
          window.transferedText = this.textContent;
        });
      }
    }
  });
  ...
  /**
   * Mise en place de l'apercu des images uploadés
   */
  let apercu_field = $(".apercu_media");
  if (apercu_field.length) {
    apercu_field.change(function () {
      let t = $(this);
      let reader = new FileReader();
      let is_image =
        this.files[0].name.match(/.*\.(jpg|jpeg|gif|png|jfif)$/gi) != null;
      reader.onload = function (e) {
        if (t.next(".preview-img").length) {
          if (!is_image) {
            t.next(".preview-img").remove();
          }
          t.next(".preview-img").attr("src", e.target.result).insertAfter(t);
        } else if (is_image) {
          let img = $("<img/>")
            .addClass("preview-img")
            .css({ maxHeight: "150px", maxWidth: "150px" })
  ...
  // {
  // $('input[type="file"]').customFileInput({
  // buttonText:'Sélectionnez votre fichier (formats : svg | png | gif | jpg | pdf)'
  // });
  // }
  ...
  $.ajax({
    ...
    success: function (reponse) {
      ...
      if ($('#droite form select[name="id_langue_page"]').val() == "") {
        $('#droite form select[name="id_rubrique"]').html(
  ...
  $.ajax({
    ...
    success: function (reponse) {
      ...
      if ($('#droite form select[name="id_langue_evenement"]').val() == "") {
        $('#droite form select[name="id_page"]').html(
          '<option value="">Sélectionner la page à associer (facultatif)</option>'
        );
      }
      $("#loader").remove();
    },
...
$.fn.customFileInput = function (options) {
  ...
  var uploadFeedback = $(
    '<a href="javascript:void(0)" class="customfile-feedback" aria-hidden="true"></a>'
  )
    .html(opts.buttonText + "<span>Parcourir</span>")
    .css({
      "background-image": "url(" + opts.icone + ")",
  ...
  if (value != "") {
    ...
    for (var nb = 0; nb < ids.length; nb++) {
      if (ids[nb] != "") {
        var ar = ids[nb].split(".");
        t.doc.innerHTML +=
          '<img src="../medias/media' + ar[0] + "_p." + ar[1] + '">';
        newVal += ar[0] + ";";
      }
    }
  ...
  observe() {
    ...
    t.doc.addEventListener("drop", (e) => {
      ...
      if (
        originalData &&
        originalData != "" &&
        originalData.match(/media([0-9]+)/i)
      ) {
        var data = originalData.match(/media([0-9]+)/i)[1];
        var ext = originalData.split(".");
        ext = ext[ext.length - 1];
        t.textarea.value =
          t.textarea.value != "" ? t.textarea.value + data + ";" : data + ";";
        t.doc.innerHTML =
          t.doc.innerHTML +
          '<img src="../medias/media' +
          data +
          "_p." +
          ext +
          '">';
...
function YSeditorObject(editor) {
  ...
  window.addEventListener("scroll", function () {
    if (imageSettings && settingsLogo) {
      ...
      settingsLogo.style.top =
        rect.top + (rect.bottom - rect.top) / 2 - 20 + "px";
      settingsLogo.style.left =
        rect.left + (rect.right - rect.left) / 2 - 20 + "px";
    }
  });

  iframe.addEventListener("paste", (event) => {
    t.textAreaSelection = t.saveSelection();
    var paste = (event.clipboardData || window.clipboardData).getData("text");
    event.preventDefault();
    paste = "<p>" + paste.split(/\n/).join("</p><p>") + "</p>";
    var parent = t.textAreaSelection.commonAncestorContainer;
    if (parent.nodeName == "#text" || !parent.tagName) {
      parent = parent.parentElement;
    }
    if (parent) {
      if (parent.nodeName == "P" && parent.textContent == "") {
        parent.outerHTML = paste;
      } else if (parent.nodeName != "DIV") {
        parent.insertAdjacentHTML("afterend", paste);
      } else {
        console.log(parent);
        parent.insertAdjacentHTML("afterbegin", paste);
      }
    } else {
      selection.deleteFromDocument();
      var toInsert = document.createElement("div");
      toInsert.outerHTML = paste;
...
YSeditorObject.prototype = {
  ...
  observe: function () {
    ...
    t.doc.addEventListener("drop", (e) => {
      if (document.caretRangeFromPoint) {
        // Chrome
        t.textAreaSelection = document.caretRangeFromPoint(
          e.clientX,
          e.clientY
        );
      ...
      var alt = "";
      if (srcFrame) {
        var img = srcFrame.contentDocument.querySelector(
          '[src="' +
            text.replace(
  ...
  execCmd: function (cmd) {
    ...
    if (cmd == "insertUnorderedList" || cmd == "insertOrderedList") {
      var parent = this.textAreaSelection.commonAncestorContainer;
      if (!parent.nodeName || parent.nodeName == "#text") {
        parent = parent.parentElement;
      }
      if (parent.nodeName != "DIV") {
        parent.outerHTML = "<ul><li>" + parent.textContent + "</li></ul>";
        return;
      }
    }
  ...
  execCmdWithVal: function (cmd, val) {
    ...
    if (cmd == "formatBlock") {
      if (
        ...
        this.textAreaSelection.commonAncestorContainer.parentNode.nodeName ==
          "LI" ||
        this.textAreaSelection.commonAncestorContainer.parentNode.nodeName ==
          "UL"
      ) {
        ...
        parentNode = this.textAreaSelection.commonAncestorContainer.parentNode;
      }
    }
  ...
  resetAllStyle: function () {
    var htmlContent = this.doc.children;
    if (htmlContent) {
      for (let el of htmlContent) {
    ...
    var aotrou = this.doc.querySelectorAll("span,i,em,b,strong,u,strike");
    if (aotrou) {
      for (var el of aotrou) {
  ...
  saveToTextarea: function () {
    var titles = this.doc.querySelectorAll("h2, h3, h4");
    for (let title of titles) {
      if (title.textContent == "") {
        title.outerHTML = "<p><br></p>";
      }
    }
  ...
  insertSeeMore: function () {
    ...
    if (this.doc.innerHTML.match('<hr class="resume">') != null) {
      this.doc.innerHTML = this.doc.innerHTML.replace(
        '<hr class="resume">',
        ""
      );
    }
  ...
  createLink: function () {
    ...
    button.addEventListener("click", function () {
      ...
      var isBlank = blank.checked ? 'target="_blank"' : 'target="_parent"';
      var a =
  ...
  image: function (src, alt) {
    ...
    if (src && src != null) {
      ...
      var img =
        ...
        "' id='" +
        id +
        "' style='display:block; float:none; margin:auto'>";
...
function adminCentre() {
  ...
  if (
    ...
    localStorage.getItem("centre") == "open"
  ) {
  ...
  bc.addEventListener("click", function (e) {
    ...
    if (this.className.match("cache") != null) {
      zg.className = zg.className
        .replace(" shadow_gauche", "")
        .replace("shadow_gauche", "");
      zc.style.left = "20%";
      zc.style.width = "25%";
      zc.style.paddingRight = "0";
...
function form_page() {
  ...
  if (
    localStorage.getItem("gabaritId") &&
    !document.getElementById("ligne_coloree")
  ) {
